...
func inicializarMockClient(mockClient *mocks.MockClient, preciosTampones, preciosToallitas []*preciosclaros.Producto,
	...
	primero := mockClient.EXPECT().ObtenerListaDePrecios(sucursales, tampones, "tampones").Return(preciosTampones, nil)
	segundo := mockClient.EXPECT().ObtenerListaDePrecios(sucursales, toallitas, "toallitas").Return(preciosToallitas, nil)
